...
df.raw <- list.files("data/pilot-data", full.names = TRUE) %>%
...
#| echo: false
#| include: false
df <- subset(df.raw, select=c(trial_index, subject_id, matrix_size, transition_matrix, sequence, trials_per_block, total_trials, practice_trials, phase, block, experiment_trial_type, trial_in_block, overall_trial, position, correct_key, response, correct, rt, questionnaire_item)) %>%
...
get_matrix_entropies <- function(matrix) {
  return(apply(matrix, 1, calculate_entropy))
}

df <- df %>%
  mutate(
    matrix_entropies = case_when(
      !is.na(transition_matrix) & !is.na(position) ~ get_matrix_entropies(fromJSON(transition_matrix)),
      TRUE ~ NA_real_
    )
  )%>%
  mutate(
    conditional_entropy = case_when(
      !is.na(transition_matrix) & !is.na(position) ~ matrix_entropies[position+1],
      TRUE ~ NA_real_
    )
  )
```
...
by_entropy_within_subs <- df %>%
  group_by(subject_id) %>%
  mutate(matrix_entropies = get_matrix_entropies(transition_matrix)) %>%
  mutate(entropy=first(matrix_entropies)[position+1]) %>%
  group_by(entropy)
  summarise(
    accuracy = mean(correct),
    mean_rt = mean(rt[correct]),
